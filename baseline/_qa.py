...
def qa_test(exp_path: str):
    ...
    for ckpt in ckpts:
        ...
        writer.add_scalar("train", accuracy_score(
            answer['qa'], pred['qa']), ckpt)


def qa_valid(exp_path: str):
    # Hyperparameters
    batch_size = 4
    with open(os.path.join(exp_path, "cfg.json"), "r") as f:
        args = json.load(f)
    ...
    # Data
    data = all_dataset("data/Valid_QA.json", "data/Valid_risk.csv")
    ...
    for ckpt in ckpts:
        ...
        for batch_data in tqdm_dldr:
            ...
            pred["qa"] = pred["qa"] + pred_qa.argmax(dim=-1).tolist()
        with open(f'qa{ckpt}.json', 'w') as f:
            json.dump(pred, f)
        # print(f"qa: {accuracy_score(answer['qa'], pred['qa'])}")
